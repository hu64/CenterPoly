...
class PolydetLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        opt = self.opt
        # hm_loss, wh_loss, off_loss, poly_loss = 0, 0, 0 , 0
        ...
        # loss = opt.hm_weight * hm_loss + opt.off_weight * off_loss + opt.poly_weight * poly_loss + opt.wh_weight * wh_loss
        # loss_stats = {'loss': loss, 'hm_loss': hm_loss,
        #               'wh_loss': wh_loss, 'off_loss': off_loss, 'poly_loss': poly_loss}  # hughes

        loss = opt.hm_weight * hm_loss + opt.off_weight * off_loss + opt.poly_weight * poly_loss
        loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'off_loss': off_loss, 'poly_loss': poly_loss}  # hughes
...
class PolydetTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        # loss_states = ['loss', 'hm_loss', 'wh_loss', 'off_loss', 'poly_loss']
